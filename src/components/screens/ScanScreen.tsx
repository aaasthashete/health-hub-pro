...
export function ScanScreen() {
  ...
  const handleCapture = () => {
    // Simulate capture
    setCapturedImages([...capturedImages, `capture-${capturedImages.length + 1}`]);
  };
      ...
      {capturedImages.length > 0 && (
        <div className="px-5 py-3 flex gap-2 overflow-x-auto">
          {capturedImages.map((img, index) => (
            <div 
              key={img}
              className="relative w-14 h-14 rounded-lg bg-card/20 shrink-0"
            >
              <button
              ...
              <div className="w-full h-full rounded-lg bg-secondary/30 flex items-center justify-center">
                <span className="text-caption text-primary-foreground">{index + 1}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="px-5 py-6 bg-foreground/95">
        ...
        {capturedImages.length > 0 && (
          <Button 
            ...
            onClick={handleScan}
          >
            Scan ({capturedImages.length} page{capturedImages.length > 1 ? 's' : ''})
          </Button>
        )}
